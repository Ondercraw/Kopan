...
export class FinancialMovement {
  ...
  @Prop({
    type: String,
    enum: [FinancialPaymentMethod.CASH, FinancialPaymentMethod.TRANSFER],
    default: null,
    index: true,
  })
  acreditadoEn:
  ...
  compraId: Types.ObjectId | null;
  @Prop({ type: Number, default: null }) compraCodigo: number | null;
  @Prop({
    type: MongooseSchema.Types.ObjectId,
    ref: 'Product',
    default: null,
  ...
  @Prop({ type: Number, min: 1, default: null }) unidadesReposicion:
    number | null;
...
export const FinancialMovementSchema =
  SchemaFactory.createForClass(FinancialMovement);
